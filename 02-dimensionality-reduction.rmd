...
```{r echo=T}
variance_explained = pcaresult$sdev^2
barplot((variance_explained[1:15] / sum(variance_explained))*100, names.arg=seq(1,15), ylab="% variance")
```
...
genenames <- rownames(D)
genenames <- genenames[4:nrow(D)]
genedata <- t(D[4:nrow(D),1:ncol(D)])
```

```{r echo=T}
tsne_model <- Rtsne(genedata, check_duplicates=FALSE, pca=TRUE, perplexity=50, theta=0.5, dims=2)
y1 <- tsne_model$Y[which(D[1,]==-1),1:2]
y2 <- tsne_model$Y[which(D[1,]==0),1:2]
y3 <- tsne_model$Y[which(D[1,]==1),1:2]
y4 <- tsne_model$Y[which(D[1,]==2),1:2]

plot(y1,type="p",col="red",xlim=c(-25, 25),ylim=c(-25, 25))
points(y2,type="p",col="black")
points(y3,type="p",col="blue")
points(y4,type="p",col="green")
...
plot(y2_0,type="p",col="tomato",xlim=c(-20, 20),ylim=c(-10, 10))
